...
<div class="row">
  <div class="col-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Edit Layanan Jasa</h4>
        <p class="card-description">Ubah detail layanan.</p>
        
        <form class="forms-sample" action="{{ route('dashboard.services.update', $service->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')
          ...
          <div class="form-group">
            ...
            <input type="number" min="0" class="form-control" id="price" name="price" value="{{ old('price', $service->price) }}">
          </div>
          <div class="form-group">
            <label>Gambar Layanan Baru (Opsional)</label>
            @if($service->image)
              <div class="mb-2">
                <img src="{{ asset('storage/' . $service->image) }}" alt="Gambar" style="max-height: 100px; border-radius: 8px;">
              </div>
            @endif
            ...
            <div class="mt-3" id="previewContainer" style="display: none;">
              <p class="text-xs text-muted mb-2">Pratinjau Gambar Baru:</p>
